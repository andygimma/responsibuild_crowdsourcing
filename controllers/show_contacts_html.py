...
def show_contacts_html(self):
    logging.debug("show_contacts_html")
    #try:
    path = os.path.join(os.path.dirname(__file__))
    path_length = path.__len__()
    final_path = path[0:path_length-11] + 'views/htmls/show_contacts_html.html'
    
    logging.debug("path = " + path)
    logging.debug("final_path = " + final_path)
    logged_in = check_login(self)
    order = "last_name"
    results, results_exist = datastore_results("Member", filters = None, inequality_filters = None, order = order, fetch_total = 1000, offset = 0, mem_key = None)
